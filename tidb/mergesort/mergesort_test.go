...
func (s *sortTestSuite) TestMergeSort(c *check.C) {
	...
	fmt.Println("Checking base case for sequential merge sort")
...
func (s *sortTestSuite) TestParMergeSort(c *check.C) {
	lens := []int{1, 3, 5, 7, 11, 13, 17, 19, 23, 29, 1024, 1 << 13, 1 << 17, 1 << 19, 1 << 20}

	fmt.Println("Checking base case for parallel merge sort")
